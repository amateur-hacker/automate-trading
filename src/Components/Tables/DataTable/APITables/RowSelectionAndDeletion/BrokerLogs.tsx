  ...
  try {
    ...
    const response = await fetch("https://nextlevelpine.com/get-broker-logs", {
      ...
      headers: {
        ...
        Authorization:
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIyNCIsImVtYWlsIjoic2FjaGluMTIzNEBnbWFpbC5jb20iLCJ1aWQiOiJyUDhsbW1NRDZ2YjZDbWJVbnY2OGZlOTRLc20yIiwicGhvbmUiOiIxMjM0NTY3ODkwIiwiVXNlcm5hbWUiOiJzYWNoaW4xMjM0IiwiaWQiOiIyNCIsIm5iZiI6MTcwNjY0NzAzNywiZXhwIjoxNzA2NzMzNDM3LCJpYXQiOjE3MDY2NDcwMzd9.s1-sYldWVlzAJ3U94njYWlQHQ8xfxWRsrIeiPDO6Bi0",
      },
      cache: "force-cache",
    });
    ...
    <table className="table table-striped table-dark" border={1} cellSpacing={5} cellPadding={5}>
      ...
      <tbody>
        {data.map((row: any) => (
          <tr key={row.s_No}>
            {brokerColumns.map((column) => (
              <td key={column.id}>{truncateText(column.selector(row), 10)}</td>
            ))}
          </tr>
        ))}
      </tbody>
